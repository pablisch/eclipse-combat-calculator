...
export const getArmies = (player, role, armies) => {
  player.forEach((ship) => {
    for (let i = 0; i < ship.quantity; i++) {
      armies.push(new Ship(role, ship.name, ship.initiative, ship.hulls, ship.computers, ship.shields, ship.cannons.ion, ship.cannons.plasma, ship.cannons.antimatter, ship.missiles))
    }
  });
}
...
export const getNextAttacker = (armiesArray) => { 
  ...
  const shipWithLowestRoundsValue = armiesArray.reduce((prevShip, currentShip) => {
    return currentShip.getRounds() < prevShip.getRounds() ? currentShip : prevShip;
  }, armiesArray[0]);
...
export const assignDamage = (targets, shooter, shots, armiesArray, allRolls) => {
  const computers = shooter.getComputers();
  for (let i = 0; i < shots.antimatterRolls.length; i++) {
    // console.log('anti:', shots.antimatterRolls[i], 'comp +', computers, 'shields -', targets[0]?.getShields() || null, 'result:', shots.antimatterRolls[i] + computers - targets[0]?.getShields());
    if (targets.length && shots.antimatterRolls[i] !== 1 && (shots.antimatterRolls[i] + computers - targets[0].getShields() >= 6 || shots.antimatterRolls[i] === 6)) {
      // console.log('hit');
      targets[0].removeHulls(4);
      if (targets[0].getHulls() <= 0) {
  ...
  for (let i = 0; i < shots.plasmaRolls.length; i++) {
    // console.log('plasma:', shots.plasmaRolls[i], 'comp +', computers, 'shields -', targets[0]?.getShields() || null, 'result:', shots.plasmaRolls[i] + computers - targets[0]?.getShields());
    if (targets.length && shots.plasmaRolls[i] !== 1 && (shots.plasmaRolls[i] + computers - targets[0].getShields() >= 6 || shots.plasmaRolls[i] === 6)) {
      // console.log('hit');
  ...
  for (let i = 0; i < shots.ionRolls.length; i++) {
    ...
    // console.log('ion:', shots.ionRolls[i], 'comp +', computers, 'shields -', targets[0]?.getShields() || null, 'result:', shots.ionRolls[i] + computers - targets[0]?.getShields());
    if (targets.length && shots.ionRolls[i] !== 1 && (shots.ionRolls[i] + computers - targets[0].getShields() >= 6 || shots.ionRolls[i] === 6)) {
      // console.log('hit');
